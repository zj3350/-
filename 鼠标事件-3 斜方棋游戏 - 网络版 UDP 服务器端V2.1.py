...
def perform_move(selected_chess, close_point, map):
    ...
    # 清除原来的棋子
    cv.create_rectangle(selected_chess[0]-20, selected_chess[1]-20, selected_chess[0]+20, selected_chess[1]+20, fill='green', outline='green')
    drawQiPan()
    ...
    for row in map:
        for cell in row:
            if cell in pieces:
                pieces[cell] += 1
    return pieces
def has_valid_moves(map, valid_moves):
    for y in range(15):
        for x in range(15):
            if map[y][x] in ['0', '1']:  # 当前位置有棋子
                piece_position = (x * 40 + 20, y * 40 + 20)  # 计算像素坐标
                moves = valid_moves.get(piece_position, [])
                for move in moves:
                    if is_valid_move(move, map):  # 检查是否有有效的移动
                        return True
    return False
def is_valid_move(move, map):
    move_x, move_y = (move[0] // 40, move[1] // 40)  # 计算数组索引
    if 0 <= move_x < 15 and 0 <= move_y < 15:  # 确保移动在棋盘范围内
        return map[move_y][move_x] == " "  # 目标位置必须是空的
    return False
